...
const Index = () => {
    ...
    return (
        <TablePageLayout
            ...
            isShowButton={true}
        >
            <TableHeader rows={['Date', 'Amount','Description', 'Status']}/>
            <tbody>
            {data && data.length ? (
                data.map((row, key) => (
                    <tr
                    ...
                    >
                        ...
                        <TableData value={row.description}/>
                        <TableData value={row.status === 0 ? 'Pending' : row.status === 1 ? 'Approved' : row.status === 3 ? 'Withdraw':'N/A'}
