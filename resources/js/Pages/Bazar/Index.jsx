                    ...
                    return (
                        ...
                        >
                            ...
                            <TableData value={status === 0 ? 'Pending' : status === 1 ? 'Approved' :'N/A' }
                                       className={`rounded-full ${status === 0 ? 'bg-danger text-danger' : status === 1 ? 'bg-success success' : 'bg-buttonColor-900 text-black dark:text-white'} text-center bg-opacity-10 py-1 px-3 text-sm `}/>
                            <TableData value={
                                <BazarScheduleUSer
                                    users={
                                        bazarSchedule &&
...
const BazarScheduleUSer = ({users}) => {
    return (
        <p className="flex items-center px-6 py-4 text-black dark:text-white">
            {users && users.length > 0
                ? users.map(({first_name, last_name}, index) => (
                    <span
                        key={index}
                        className={`bg-success  mr-2 px-2.5 py-0.5 rounded`}
